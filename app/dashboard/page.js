...
export default function DashboardPage() {
  ...
  async function cargarDatos() {
    ...
    // Todas las consultas de abajo son independientes entre sí (ninguna necesita
    // el resultado de otra), así que se piden todas a la vez con Promise.all
    // en vez de una detrás de otra. Esto es lo que hace que el Dashboard cargue
    // en el tiempo de la consulta más lenta, no en la suma de todas.

    let qCuentas = supabase.from('v_saldo_cuentas').select('*').order('nombre');
    ...
    // Presupuesto solo se calcula en vista Mensual (ver nota más abajo en el JSX)
    let qCategoriasPresupuesto = Promise.resolve({ data: [] });
    let qGastoPresupuesto = Promise.resolve({ data: [] });
    ...
    if (tipoPeriodo === 'Mensual') {
      ...
      if (proyectoId) qc = qc.eq('proyecto_id', proyectoId);
      qCategoriasPresupuesto = qc;
      ...
      let qg = supabase
        ...
        .lte('fecha', rango.fin);
      if (proyectoId) qg = qg.eq('proyecto_id', proyectoId);
      qGastoPresupuesto = qg;
    }

    const [resCuentas, resCapital, resDeuda, resGasto, resCategoriasPresupuesto, resGastoPresupuesto] =
      await Promise.all([qCuentas, qCapital, qDeuda, qGasto, qCategoriasPresupuesto, qGastoPresupuesto]);

    setSaldoCuentas(resCuentas.data || []);
    setSaldoCapital(resCapital.data || []);
    setSaldoDeuda(resDeuda.data || []);

    if (tipoPeriodo === 'Mensual') {
      const gastadoPorCategoria = {};
      (resGastoPresupuesto.data || []).forEach((row) => {
        gastadoPorCategoria[row.categoria_id] = (gastadoPorCategoria[row.categoria_id] || 0) + Number(row.valor);
      });

      setPresupuesto(
        (resCategoriasPresupuesto.data || []).map((c) => ({
          categoria_id: c.id,
          nombre: c.nombre,
        ...
        <>
          ...
          <div className="bg-white rounded-xl p-3 shadow-sm mb-6" style={{ height: 220 }}>
            {gastoPorCategoria.length === 0 ? (
              <p className="text-sm text-gray-400">Sin gastos en este periodo.</p>
            ) : (
