...
export default function CapitalPage() {
  ...
  const total = saldoCapital.reduce((acc, c) => acc + Number(c.saldo), 0);
  ...
  return (
    <main className="min-h-screen p-4 max-w-md mx-auto pb-24">
      <h1 className="text-xl font-semibold mb-4">Capital</h1>
      ...
      {cargando ? (
        ...
        <>
          <div className="bg-white rounded-xl p-4 shadow-sm mb-6">
            ...
            <p className="text-2xl font-semibold text-blue-600">{formatoCOP.format(total)}</p>
          </div>

          <h2 className="text-sm font-medium text-gray-600 mb-2">Detalle</h2>
          <div className="bg-white rounded-xl p-3 shadow-sm mb-6 space-y-2">
            {saldoCapital.length === 0 && <p className="text-sm text-gray-400">Sin capital registrado.</p>}
            {saldoCapital.map((c, i) => (
              <div key={i} className="flex justify-between text-sm">
                <span>
                  {c.categoria}
                  {c.subcategoria ? ` › ${c.subcategoria}` : ''}
                </span>
                <span className="font-medium text-blue-600">{formatoCOP.format(c.saldo)}</span>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            ...
            {movimientos.map((m) => (
              <div key={m.id} className="bg-white rounded-xl p-3 shadow-sm flex justify-between items-center">
                ...
                <p className="text-sm font-semibold text-blue-600 shrink-0">{formatoCOP.format(m.valor)}</p>
              </div>
            ))}
          </div>
        </>
